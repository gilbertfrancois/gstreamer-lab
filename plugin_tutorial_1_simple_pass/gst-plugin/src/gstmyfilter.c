...
static void gst_my_filter_class_init(GstMyFilterClass *klass) {
  g_print("I have initialized my class\n");
...
static gboolean myfilter_init(GstPlugin *myfilter) {
  GST_DEBUG_CATEGORY_INIT (gst_my_filter_debug, DI_PLUGIN_NAME,
                           0, "Template myfilter");
  g_print("I have initialized my plugin\n");
  return gst_element_register(myfilter, DI_PLUGIN_NAME, GST_RANK_NONE,
                              GST_TYPE_MYFILTER);
}
