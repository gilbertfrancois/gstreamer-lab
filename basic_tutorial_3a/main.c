...
int main(int argc, char *argv[]) {
  ...
  if (!gst_element_link(data.source, data.qtdemux)) {
    g_printerr("Source could not be linked to demuxer.\n");
    return -1;
  }
  if (!gst_element_link(data.h264parse, data.avdec_h264)) {
    g_printerr("Parser could not be linked to decoder.\n");
    return -1;
  }
  if (!gst_element_link(data.avdec_h264, data.videosink)) {
    g_printerr("Decoder could not be linked to the video sink.\n");
    return -1;
  }
  ...
  g_signal_connect(data.qtdemux, "pad-added", G_CALLBACK(pad_added_handler),
                   &data);
...
static void pad_added_handler(GstElement *src, GstPad *new_pad,
  ...
  if (g_str_has_prefix(new_pad_type, "video/x-h264")) {
    sink_pad = gst_element_get_static_pad(data->h264parse, "sink");
  } else {
